...
struct CustomPromptButton: View {
    ...
    @Binding var selectedPrompt: UUID?
    @State private var showPromptMenu = false
    
    // 添加初始化方法支持绑定
    init(selectedPrompt: Binding<UUID?> = .constant(nil)) {
    ...
    private var isSelectedPromptValid: Bool {
        ...
        return preferences.customPrompts.contains { $0.id == selectedPrompt }
    }
    
    /// 获取当前选中提示词的名称用于显示
    private var selectedPromptName: String? {
        guard let selectedPrompt = selectedPrompt else { return nil }
        return preferences.getCustomPrompt(by: selectedPrompt)?.name
    }

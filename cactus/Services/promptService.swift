...
"""
    ...
    public func getSystemMessageForDict() -> String {
        ...
        case "zh-CN":
            return """
you是一位资深语言学专家与辞书编纂者，请参照《现代汉语词典》《辞海》等权威辞书的体例，对指定中文词语进行权威释义。释义应语言规范、用词严谨、结构清晰，风格偏重工具性和学术性，适用于辞典条目撰写。
...
"""
        case "zh-TW":
            return """
you是一位語言學專家與辭書編纂者，請參照《國語辭典重編本》、《教育部重編國語辭典修訂本》等權威辭書的體例，對指定中文詞語進行權威釋義。釋義應語言規範、用詞嚴謹、結構清晰，風格偏重工具性和學術性，適用於辭典條目撰寫。
...
### 📚 語源
"りんご"は中国語"林檎"から借用された語で、もともとは野生の果実を指していた。
"""
        case "ko-KR":
            return """
...
### 📚 어원
"사과"는 중국어 "沙果"에서 유래된 말로, 원래는 야생 과실을 가리켰다.
...
### 📚 Etymology
From Old English "æppel", from Proto-Germanic "*aplaz", ultimately from Proto-Indo-European "*h₂ébōl".
"""
        }
    }
